...
import ThumbNailImage from './components/ThumbNailImage/index'
import './App.css'
...
class App extends Component {
  state = {
    ...
    score: 0,
    time: 60,
    generatedUrl: imagesList[0].imageUrl,
    gameOver: false,
  }

  componentDidMount() {
    const timerId = setInterval(this.onStartTimer, 1000)
  }
  ...
  onStartTimer = () => {
    const {time, gameOver} = this.state
    if (time <= 0 || gameOver) {
      clearInterval(this.timerId)
    } else {
  ...
  render() {
    const {isActiveTab, score, time, generatedUrl, gameOver} = this.state
    const thumbNails = imagesList.filter(img => img.category === isActiveTab)
    let gameView
      ...
      gameView = (
        <div className="bg-container">
          ...
          <div className="game-images-container">
            <img src={generatedUrl} alt="match" className="match-image" />
            <ul className="tabs-container">
              {tabsList.map(tab => (
                <TabItem
                  key={tab.tabId}
                  tabsList={tab}
                  isActive={isActiveTab === tab.tabId}
            ...
            <ul className="images-container">
              {thumbNails.map(img => (
                <ThumbNailImage
                  ...
                  onClickedThumbNail={this.onClickedThumbNail}
                />
              ))}
            </ul>
            <ul className="dummy-lists">
              <li>a</li>
              <li>b</li>
              <li>c</li>
            </ul>
